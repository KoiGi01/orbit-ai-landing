...
export async function getWorkspaceTrendsForClient(authorization, database, rawDays) {
  const session = await authenticateSession(authorization);
  if (!session.orgId) throw new ControlError(409, 'organization_required');
  return getWorkspaceTrends(requireDatabase(database), session.orgId, rawDays);
}
