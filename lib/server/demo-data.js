...
// The first calls the generator builds are the unresolved ones: four that need
// a follow-up and two that went to voicemail.
const SEEDED_PENDING_CALLS = 6;
...
function buildCalls(contacts, catalog, now) {
  ...
  return calls.map((call, index) => {
    ...
    // the decision queue has something real behind it; two more land in
    // voicemail so the outcome donut shows that slice too. The dispositions are
    // the ones upsertCallAnalyzed actually writes, so seeded activity is
    // bucketed by exactly the same rules as production activity.
    const missed = index === 4 || index === 5;
    const followUp = index < 4 || missed;
    ...
    return {
      ...
      durationSeconds,
      followUp,
      urgency: missed ? 'urgent' : index === 0 ? 'urgent' : followUp ? 'high' : 'normal',
      disposition: missed ? 'voicemail' : followUp ? 'unsuccessful' : 'completed',
      intent: service,
      summary: followUp
...
function buildAppointments(contacts, calls, catalog, now) {
  ...
  return slots.map((slot, index) => {
    ...
    return {
      ...
      contactId: contact.id,
      // Skip the calls that are already unresolved or in voicemail: an
      // appointment outranks both when the donut buckets a call, and stealing
      // them would empty the follow-up and voicemail slices.
      callId: calls[SEEDED_PENDING_CALLS + index].id,
