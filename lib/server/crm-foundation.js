...
export async function getConnectedGoogleCalendarId(database, workspaceId) {
  const result = await database.query(
    `
      select external_account_id
      from app.integration_connections
      where workspace_id = $1 and provider_key = 'google_calendar' and status = 'connected'
      limit 1
    `,
    [workspaceId],
  );
  return result.rows[0]?.external_account_id || null;
}
...
function clampTrendDays(rawDays) {
...
/**
 * Per-bucket call volume for the workspace's own chart.
 *
 * getWorkspaceActivity caps at the 20 most recent calls because it feeds a
 * list; a chart asking "how did the week go" cannot be drawn from a truncated
 * list without lying as soon as the twenty-first call lands, so the buckets are
 * aggregated in Postgres instead.
 *
 * The series is generated rather than derived from the rows, so a day with no
 * calls is a zero in the chart instead of a missing bar. Bucketing happens in
 * the workspace's own timezone: a 19:00 call in Mexico City is 01:00 UTC and
 * would otherwise be filed under tomorrow.
 */
export async function getWorkspaceTrends(database, rawClerkOrganizationId, rawDays = 7) {
  const orgId = clerkOrganizationId(rawClerkOrganizationId);
  const days = clampTrendDays(rawDays);
  const granularity = days === 1 ? 'hour' : 'day';
  const buckets = days === 1 ? 24 : days;
  const empty = { days, granularity, timezone: null, points: [], totals: { calls: 0, needsAttention: 0, avgDurationSeconds: 0 } };
  ...
  const keyFormat = granularity === 'hour' ? 'YYYY-MM-DD"T"HH24:00' : 'YYYY-MM-DD';

  // `granularity` is one of two literals chosen above, never user input, so
  // interpolating it into the interval and date_trunc is safe; every value that
  // comes from outside stays a bound parameter.
  const result = await database.query(
    `
      with anchor as (
        select date_trunc('${granularity}', (now() at time zone $2)) as newest
      ),
      series as (
        select generate_series(
          (select newest from anchor) - make_interval(${granularity}s => $3 - 1),
          (select newest from anchor),
          make_interval(${granularity}s => 1)
        ) as bucket
      )
      select
        to_char(s.bucket, '${keyFormat}') as bucket_key,
        count(c.id)::int as calls,
        count(c.id) filter (where c.follow_up_required)::int as needs_attention,
        coalesce(avg(nullif(c.duration_seconds, 0)), 0)::float as avg_duration_seconds
      from series s
      left join app.calls c
        on c.workspace_id = $1
       and date_trunc('${granularity}', (coalesce(c.started_at, c.created_at) at time zone $2)) = s.bucket
      group by s.bucket
      order by s.bucket
    `,
    [workspace.id, timezone, buckets],
  );
  ...
  // Averaging the bucket averages would weight a quiet hour like a busy one.
  const totalCalls = points.reduce((sum, point) => sum + point.calls, 0);
  const weighted = points.reduce((sum, point) => sum + point.avgDurationSeconds * point.calls, 0);
  ...
  return {
    ...
    totals: {
      calls: totalCalls,
      needsAttention: points.reduce((sum, point) => sum + point.needsAttention, 0),
      avgDurationSeconds: totalCalls ? Math.round(weighted / totalCalls) : 0,
    },
  };
}
...
// A window of 0 is a bad request, not an absent one: it is raised to the
// minimum rather than quietly falling back to the default, which would answer a
// different question than the one asked.
function clampActivityDays(rawDays) {
  if (rawDays === null || rawDays === undefined || rawDays === '') return DEFAULT_ACTIVITY_DAYS;
...
// One aggregate for the whole console. The admin list shows up to 100
// Locations, so a per-Location round trip would make the page unusable; this
// answers every row from a single scan and the caller indexes it by
// organization. Days are clamped because the value reaches here from a query
// string.
export async function getAdminActivitySummary(database, rawDays = 14) {
  const days = clampActivityDays(rawDays);
  const result = await database.query(
    `
      select
        w.clerk_organization_id as organization_id,
        to_char(date_trunc('day', coalesce(c.started_at, c.created_at)), 'YYYY-MM-DD') as day,
        count(*)::int as calls,
        coalesce(avg(nullif(c.duration_seconds, 0)), 0)::float as avg_duration_seconds
      from app.calls c
      join app.workspaces w on w.id = c.workspace_id
      where w.archived_at is null
        -- Seeded demo activity belongs to the client's dashboard, not to
        -- AutiveX's own operating numbers.
        and c.demo_batch_id is null
        and coalesce(c.started_at, c.created_at) >= (now() - make_interval(days => $1))
      group by 1, 2
    `,
    [days],
  );

  // The sparkline needs one point per day including the silent ones, and a gap
  // in the rows means zero calls, not a missing day.
  const timeline = [];
  for (let offset = days - 1; offset >= 0; offset -= 1) {
    const date = new Date();
    date.setUTCHours(0, 0, 0, 0);
    date.setUTCDate(date.getUTCDate() - offset);
    timeline.push(date.toISOString().slice(0, 10));
  }
  const recentCutoff = timeline[Math.max(timeline.length - 7, 0)];

  const byOrganization = new Map();
  for (const row of result.rows) {
    const id = String(row.organization_id || '');
    if (!id) continue;
    if (!byOrganization.has(id)) byOrganization.set(id, { days: new Map(), weighted: 0, counted: 0 });
    const entry = byOrganization.get(id);
    const calls = Number(row.calls) || 0;
    entry.days.set(String(row.day), calls);
    // Averaging the daily averages would weight a quiet day like a busy one.
    entry.weighted += Number(row.avg_duration_seconds || 0) * calls;
    entry.counted += calls;
  }

  const summary = {};
  for (const [id, entry] of byOrganization) {
    const series = timeline.map((day) => ({ day, calls: entry.days.get(day) || 0 }));
    summary[id] = {
      series,
      totalCalls: series.reduce((sum, point) => sum + point.calls, 0),
      recentCalls: series.filter((point) => point.day >= recentCutoff).reduce((sum, point) => sum + point.calls, 0),
      averageDurationSeconds: entry.counted ? Math.round(entry.weighted / entry.counted) : 0,
    };
  }
  return { days, timeline, summary };
}
