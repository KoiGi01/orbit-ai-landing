...
/**
 * What happened to the workspace's calls over a window.
 *
 * The four categories are cut from what the pipeline actually records, not from
 * what would be nice to show: upsertCallAnalyzed only ever writes 'voicemail',
 * 'unsuccessful' or 'completed', and app.calls.intent is never filled for a real
 * call, so "why did they call" cannot be answered until the Retell agent is
 * given post-call analysis fields. "Did it end in an appointment" can be, from
 * app.appointments.call_id, which n8n fills when a booking is confirmed.
 *
 * The categories are mutually exclusive and their precedence is booked > missed
 * > needs follow-up > resolved, so a voicemail that also booked counts once, as
 * a booking. They sum to totals.calls by construction, which is what lets the
 * donut and the KPI strip agree on screen.
 */
export async function getWorkspaceOutcomes(database, rawClerkOrganizationId, rawDays = 7) {
  const orgId = clerkOrganizationId(rawClerkOrganizationId);
  const days = clampWindowDays(rawDays);
  const empty = {
    days,
    outcomes: { booked: 0, missed: 0, needsFollowUp: 0, resolved: 0 },
  ...
  const result = await database.query(
    `
      with windowed as (
        ...
        from app.calls c
        where c.workspace_id = $1
          and coalesce(c.started_at, c.created_at) >= now() - make_interval(days => $2)
      )
      select
        count(*)::int as calls,
        count(*) filter (where booked)::int as booked,
        count(*) filter (
          where not booked and disposition is not distinct from 'voicemail'
        )::int as missed,
        count(*) filter (
          where not booked
  ...
  const outcomes = {
    ...
    needsFollowUp: Number(row.needs_follow_up) || 0,
  ...
  return {
    ...
    totals: {
      ...
      // The KPI card and the donut have to tell the same story, so "needs
      // attention" is exactly the two categories the donut paints as needing a
      // person rather than a separate count of follow_up_required.
      needsAttention: outcomes.missed + outcomes.needsFollowUp,
