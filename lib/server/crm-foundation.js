  ...
  return database.transaction(async (transaction) => {
    const workspaceResult = await transaction.query(
      `
        insert into app.workspaces (
          ...
          status,
          settings
        ) values ($1, $2, $3, $4, 'es-MX', 'testing', $5::jsonb)
    ...
    await transaction.query(
      `
        ...
        select $1::uuid, $2::uuid, 'workspace.manual_onboarding_created', 'workspace', $2::text, $3::jsonb
        where not exists (
          select 1
          from app.audit_log
  ...
  return database.transaction(async (transaction) => {
    ...
    const agentResult = await transaction.query(
      `
        insert into app.voice_agents (
          ...
          is_default,
          settings
        ) values ($1, $2, 'retell', $3, $4, $5, 'staging', 'draft', true, $6::jsonb)
    ...
    await transaction.query(
      `
        ...
        select $1::uuid, $2::uuid, 'voice_agent.draft_created', 'voice_agent', $3::text, $4::jsonb
        where not exists (
          select 1
          from app.audit_log
          where workspace_id = $2::uuid
    ...
    const agentResult = await transaction.query(
      `
        insert into app.voice_agents (
          ...
          webhook_verified,
          settings
        ) values ($1, $2, 'retell', $3, $4, $5, 'production', 'testing', true, $6, $7, $8, $9, $10::jsonb)
        on conflict (provider, external_agent_id) do update
        set external_agent_version = excluded.external_agent_version,
            display_name = excluded.display_name,
    ...
    await transaction.query(
      `
        insert into app.audit_log (
          ...
          target_id,
          metadata
        ) values ($1, $2, 'voice_agent.manual_provisioning_saved', 'voice_agent', $3, $4::jsonb)
      `,
      [
        randomUUID(),
        workspace.id,
