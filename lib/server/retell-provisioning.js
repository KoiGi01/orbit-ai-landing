...
function beginMessageFor(profile) {
  ...
  return profileValue(profile, 'greeting', `Hola, gracias por llamar a ${name}. Soy Lucía, la asistente virtual. ¿En qué puedo ayudarle?`);
}
...
export function buildRetellBusinessPrompt(rawProfile = {}) {
  ...
  const schedulingRules = hasCalendar
    ? `- Tienes una herramienta de agenda conectada: úsala para consultar disponibilidad real antes de ofrecer un horario, y para crear la cita en cuanto la persona confirme fecha y hora.
- No digas que una cita quedó agendada hasta que la herramienta responda que la creó. Si la herramienta falla o no hay disponibilidad, ofrece otro horario o toma el recado para que el equipo confirme.
- Repite solamente los datos críticos antes de terminar: nombre, teléfono y fecha y hora confirmadas.`
    : `- Todavía no tienes una agenda conectada: toma nombre, teléfono, servicio, fecha y horario preferidos; explica que el equipo confirmará.
...
Servicios: ${services}
Horario: ${hours}${offDays.length ? `\nExcepciones de horario: ${offDays.join('; ')}` : ''}
...
- Si preguntan cuánto dura o cuánto cuesta un servicio, usa la duración y el costo de la lista de Servicios. Si un servicio no tiene esa información ahí, dilo con honestidad en vez de inventar un número.
${schedulingRules}
