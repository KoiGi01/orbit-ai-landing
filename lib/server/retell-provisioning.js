...
export const RETELL_PROMPT_TEMPLATE_VERSION = 'autivex-es-mx-v4';
...
function beginMessageFor(profile) {
  ...
  return profileValue(profile, 'greeting', `${name}, le atiende Lucía. ¿En qué le ayudo?`);
}

function calendarTool(profile, env) {
  const calendarId = cleanText(profile?.calendarId, 240);
  const workspaceId = cleanText(profile?.workspaceId, 80);
  const appUrl = String(env.AUTIVEX_APP_URL || '').trim().replace(/\/$/, '');
  const directUrl = workspaceId && /^https:\/\//.test(appUrl)
    ? `${appUrl}/api/google/calendar/tool?workspaceId=${encodeURIComponent(workspaceId)}`
    : '';
  const url = directUrl || String(env.RETELL_CALENDAR_WEBHOOK_URL || 'https://autivex-ai.app.n8n.cloud/webhook/retell-calendar').trim();
  ...
  return {
    ...
    parameters: {
      type: 'object', required: ['action', 'calendarId', ...(workspaceId ? ['workspaceId'] : [])],
      properties: {
        ...
        ...(workspaceId ? { workspaceId: { type: 'string', const: workspaceId, description: 'Workspace asignado a esta Location.' } } : {}),
...
// Advanced per-agent settings, editable from the internal console. Every
// allowed value here was read back from the Retell API rather than assumed:
// an invalid voice_emotion or stt_mode is rejected with the enum in the error
// body, and "" is NOT a member -- clearing an emotion means sending null.
export const RETELL_VOICE_EMOTIONS = ['calm', 'sympathetic', 'happy', 'sad', 'angry', 'fearful', 'surprised'];
export const RETELL_STT_MODES = ['fast', 'accurate', 'custom'];

// The single source of truth for how a fresh agent sounds. agentPayload() and
// the console's "restore defaults" both read from here, so a new Location and
// a reset Location end up identical.
export const DEFAULT_AGENT_RUNTIME = {
  ...
  beginMessageDelayMs: 450,
  voiceEmotion: '',
  sttMode: 'fast',
  modelTemperature: 0.7,
};

const AGENT_RUNTIME_RANGES = {
  voiceSpeed: [0.5, 2],
  voiceTemperature: [0, 2],
  responsiveness: [0, 1],
  interruptionSensitivity: [0, 1],
  backchannelFrequency: [0, 1],
  beginMessageDelayMs: [0, 5000],
...
// Clamps rather than rejects out-of-range numbers: a slider that sends 3.0 for
// a 0-2 field is a UI bug, not an operator decision worth failing the save
// over. Unknown enum values DO throw, because silently picking a different
// voice emotion than the one that was chosen would be worse than an error.
export function normalizeAgentRuntimeSettings(raw = {}, current = {}) {
  const base = { ...DEFAULT_AGENT_RUNTIME, ...current };
  const settings = {};

  for (const [key, range] of Object.entries(AGENT_RUNTIME_RANGES)) {
    settings[key] = raw[key] === undefined || raw[key] === null || raw[key] === ''
      ? clampedNumber(base[key], range, DEFAULT_AGENT_RUNTIME[key])
      : clampedNumber(raw[key], range, base[key]);
  }

  settings.enableBackchannel = raw.enableBackchannel === undefined
    ? base.enableBackchannel !== false
    : raw.enableBackchannel === true;

  const emotion = cleanText(raw.voiceEmotion === undefined ? base.voiceEmotion : raw.voiceEmotion, 20).toLowerCase();
  if (emotion && !RETELL_VOICE_EMOTIONS.includes(emotion)) throw new Error('invalid_voice_emotion');
  settings.voiceEmotion = emotion;

  const sttMode = cleanText(raw.sttMode === undefined ? base.sttMode : raw.sttMode, 20).toLowerCase() || 'fast';
  if (!RETELL_STT_MODES.includes(sttMode)) throw new Error('invalid_stt_mode');
  settings.sttMode = sttMode;

  return settings;
}

// Operator-authored text that rides along with the generated prompt. An
// operator pastes in whatever a client dictated over the phone, so it gets the
// same stripping the public demo applies to business text -- it must not be
// able to rewrite the rules that follow it. Line breaks survive; this is
// prose the agent reads, not a single field.
export function safePromptAddition(value) {
  const lines = String(value ?? '')
    .replace(/[{}<>`]/g, '')
    .replace(/\b(ignore|ignora|olvida|disregard|system prompt|developer message|instrucciones anteriores)\b/gi, '')
    .split(/\r?\n/)
...
// Pushes the advanced settings onto an existing agent. Two narrow PATCHes,
// same style as every other update path in this module: the voice and pacing
// live on the agent, the model temperature on its LLM, and nothing outside
// these fields is touched -- so hand-tuning done directly in Retell on any
// other field survives.
export async function updateRetellAgentRuntime(raw = {}, dependencies = {}) {
  ...
  const settings = normalizeAgentRuntimeSettings(raw.settings, raw.settings);

  const agent = await retellRequest(fetchImpl, apiKey, `/update-agent/${agentId}`, {
    method: 'PATCH',
    body: JSON.stringify({
      voice_speed: settings.voiceSpeed,
      voice_temperature: settings.voiceTemperature,
      responsiveness: settings.responsiveness,
      interruption_sensitivity: settings.interruptionSensitivity,
      enable_backchannel: settings.enableBackchannel,
      backchannel_frequency: settings.backchannelFrequency,
      begin_message_delay_ms: settings.beginMessageDelayMs,
      stt_mode: settings.sttMode,
      // Retell rejects "" for this field; null is how an emotion is cleared.
      voice_emotion: settings.voiceEmotion || null,
    }),
  });

  await retellRequest(fetchImpl, apiKey, `/update-retell-llm/${llmId}`, {
    method: 'PATCH',
    body: JSON.stringify({ model_temperature: settings.modelTemperature }),
  });

  return { settings, agentVersion: String(agent?.version ?? '') };
}
...
export function buildRetellBusinessPrompt(rawProfile = {}) {
  ...
  const extraInstructions = safePromptAddition(rawProfile.extraInstructions);
...
Hay otras llamadas esperando. Atiende bien, resuelve lo necesario y pasa a la siguiente sin alargar ésta. Nunca le digas a quien llama que tienes prisa, más llamadas o mucho trabajo.
...
# Cómo hablas
Suena como una operadora ocupada y competente, no como una anfitriona ni como una asistente conversacional.
- Responde normalmente con una sola frase corta. En un turno simple, no pases de quince palabras. Usa máximo dos frases antes de volver a escuchar.
- Una idea y una sola pregunta por turno. Después cállate.
- Contesta únicamente lo que preguntaron. No des contexto, recomendaciones ni detalles extra salvo que sean necesarios para completar la solicitud.
- Después de contestar, haz la pregunta mínima que haga avanzar la llamada.
- No llenes silencios, no hagas plática y no intentes mantener la conversación viva.
- Si el asunto ya quedó resuelto, confirma lo que sigue, despídete y termina la llamada. No la reabras preguntando si necesita algo más.
- Si hace falta confirmar datos críticos, puedes usar más palabras, pero sin explicar tu proceso.
...
"Permítame tantito." / "Ahorita le reviso." / "¿Me repite su apellido, por favor?" / "¿Me lo deletrea?" / "Sale, entonces lo espero el martes." / "¿Me confirma su teléfono?" / "Listo. Que tenga buen día."
...
${extraInstructions ? `# Indicaciones del negocio
Esto lo pidió el negocio directamente. Respétalo mientras no choque con las reglas de abajo; si choca, mandan las de abajo.
${extraInstructions}
...
# Cómo cuelgas
En cuanto el asunto quede resuelto, confirma en una frase qué sigue, despídete corto y usa end_call. No resumas la llamada ni preguntes si necesita algo más.`;
}
...
function agentPayload(template, llmId, profile, env) {
  ...
  const payload = {
    ...
    language: 'es-419',
    voice_temperature: DEFAULT_AGENT_RUNTIME.voiceTemperature,
    voice_speed: DEFAULT_AGENT_RUNTIME.voiceSpeed,
    volume: 1.0,
    enable_dynamic_voice_speed: true,
    responsiveness: DEFAULT_AGENT_RUNTIME.responsiveness,
    enable_dynamic_responsiveness: true,
    interruption_sensitivity: DEFAULT_AGENT_RUNTIME.interruptionSensitivity,
    ...
    enable_backchannel: DEFAULT_AGENT_RUNTIME.enableBackchannel,
    backchannel_frequency: DEFAULT_AGENT_RUNTIME.backchannelFrequency,
    backchannel_words: ['mhm', 'ajá', 'sí'],
    begin_message_delay_ms: DEFAULT_AGENT_RUNTIME.beginMessageDelayMs,
    stt_mode: DEFAULT_AGENT_RUNTIME.sttMode,
...
export async function createRetellAgentDraft(raw = {}, dependencies = {}) {
  ...
  const llm = await retellRequest(fetchImpl, apiKey, '/create-retell-llm', {
    ...
    body: JSON.stringify({
      ...
      model_temperature: DEFAULT_AGENT_RUNTIME.modelTemperature,
      tool_call_strict_mode: true,
      start_speaker: 'agent',
      begin_message: beginMessageFor(profile),
      general_prompt: buildRetellBusinessPrompt(profile),
      general_tools: [...(schedulingTool ? [schedulingTool] : []), { type: 'end_call', name: 'end_call', description: 'Finaliza la llamada en cuanto el asunto quede resuelto y ya te hayas despedido brevemente.' }],
...
// A voice imported through Retell's "Add custom voice" flow -- how an ElevenLabs
// community voice becomes usable here -- comes back with voice_type "custom" and
// NO accent, gender or age at all. Judging it by accent therefore hid every voice
// we had deliberately imported, so custom voices are trusted instead: someone put
// them in this workspace on purpose.
function isSelectableVoice(voice) {
  if (voice?.voice_type === 'custom') return true;
  return SPANISH_VOICE_ACCENTS.has(String(voice?.accent || '').toLowerCase());
}
...
export async function listRetellSpanishVoices(dependencies = {}) {
  ...
  // Our own endorsement, keyed by the name the voice was imported under, and
  // deliberately NOT Retell's own recommended flag -- that one is true for six
  // platform voices at once, which is a catalog hint rather than the single
  // "pick this one" the dashboard needs.
  const recommendedNames = new Set(
    COMMUNITY_VOICE_IMPORTS.filter((entry) => entry.recommended).map((entry) => entry.voiceName),
  );
  return (Array.isArray(voices) ? voices : [])
    .filter(isSelectableVoice)
    .map(serializeSpanishVoice)
    .filter((voice) => voice.id && voice.name && voice.provider)
    .map((voice) => ({ ...voice, recommended: recommendedNames.has(voice.name) }))
    // The recommended voice leads, then Mexican accents: the product sells into
    // Mexico, and the rest of the Spanish voices are the exception a client opts into.
    .sort((a, b) => (
...
// ElevenLabs community voices the picker should always offer. Retell mints its own
// custom_voice_<hash> on import and never echoes provider_voice_id back from
// /list-voices or /get-voice, so the only way to recognise a voice we already
// imported is the exact name we imported it under. Treat voiceName as a join key:
// if someone renames a voice in the Retell dashboard, update it here in the same
// change, or the next sync will import a duplicate.
export const COMMUNITY_VOICE_IMPORTS = [
  { providerVoiceId: 'nbcvT3C2tyOd2OsRAtUf', voiceName: 'Maya' },
  // Imported by hand before this list existed, so it is pinned to the community
  // name it landed with rather than a tidier one.
  { providerVoiceId: 'nTkjq09AuYgsNR8E4sDe', voiceName: 'Cristina Campos - Natural Conversations', recommended: true },
];

export async function searchRetellCommunityVoice(providerVoiceId, dependencies = {}) {
  const env = dependencies.env || process.env;
  const id = cleanText(providerVoiceId, 160);
  if (!id) throw new Error('missing_provider_voice_id');
  const result = await retellRequest(dependencies.fetchImpl || fetch, retellApiKey(env), '/search-community-voice', {
    method: 'POST',
    body: JSON.stringify({ search_query: id, voice_provider: 'elevenlabs' }),
  });
  const matches = Array.isArray(result?.voices) ? result.voices : [];
  return matches.find((voice) => voice?.provider_voice_id === id) || null;
}

// Idempotent: every voice in COMMUNITY_VOICE_IMPORTS ends up in the workspace,
// and one already there is left untouched rather than imported twice.
export async function ensureRetellCommunityVoices(dependencies = {}) {
  const env = dependencies.env || process.env;
  const fetchImpl = dependencies.fetchImpl || fetch;
  const catalog = await retellRequest(fetchImpl, retellApiKey(env), '/list-voices');
  const importedNames = new Set(
    (Array.isArray(catalog) ? catalog : [])
      .filter((voice) => voice?.voice_type === 'custom')
      .map((voice) => String(voice?.voice_name || '')),
  );

  const results = [];
  for (const entry of COMMUNITY_VOICE_IMPORTS) {
    if (importedNames.has(entry.voiceName)) {
      results.push({ ...entry, status: 'present' });
      continue;
    }
    const community = await searchRetellCommunityVoice(entry.providerVoiceId, { env, fetchImpl });
    if (!community) {
      results.push({ ...entry, status: 'not_found' });
      continue;
    }
    const added = await retellRequest(fetchImpl, retellApiKey(env), '/add-community-voice', {
      method: 'POST',
      body: JSON.stringify({
        provider_voice_id: entry.providerVoiceId,
        voice_name: entry.voiceName,
        voice_provider: 'elevenlabs',
        // ElevenLabs is the one provider that requires the owner's id, and it is
        // only discoverable through the community search above.
        public_user_id: community.public_user_id,
      }),
    });
    results.push({ ...entry, status: 'imported', voiceId: cleanText(added?.voice_id, 160) || null });
  }
  return results;
}
...
export async function updateRetellCalendarIntegration(raw = {}, dependencies = {}) {
  ...
  const llmId = cleanText(raw.llmId, 128);
  if (!llmId) throw new Error('missing_retell_llm_id');
  const schedulingTool = calendarTool({ calendarId: raw.calendarId, workspaceId: raw.workspaceId }, env);
  ...
  await retellRequest(fetchImpl, retellApiKey(env), `/update-retell-llm/${llmId}`, {
    method: 'PATCH',
    body: JSON.stringify({ general_tools: [...retained, schedulingTool, { type: 'end_call', name: 'end_call', description: 'Finaliza la llamada en cuanto el asunto quede resuelto y ya te hayas despedido brevemente.' }] }),
  });
